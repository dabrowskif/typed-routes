import { fakeProcessArgs, fakeRequiredProcessArgs } from '../../../../test/fixtures/cli';
import { CLI } from './cli';
...
describe('CLI', () => {
  ...
  it('should return process.exit code as undefined because all required options are passed', () => {
    ...
    cli.generateProgramOptions(['node', 'index.js', '-r', 'someRootDir/', '-f', 'sveltekit']);
  ...
  ])('should return process.exit code as 1 because $label', async ({ args }) => {
    let exitCode: number | undefined = undefined;
    // @ts-expect-error - for testing purposes
    process.exit = (code: number) => {
      exitCode = code;
  ...
  ])('it should throw because wrong argument $label provided', async ({ args }) => {
